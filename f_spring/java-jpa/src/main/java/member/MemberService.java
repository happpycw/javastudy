...
public class MemberService {
    ...
    List<Member> findWithCreatedAt(LocalDateTime start, LocalDateTime end){
        try(EntityManager em = jpaTemplate.createEntityManager()){
    ...
    void merge(Member member){
        ...
        try{
            tx.begin();
            // 엔티티 매니저가 관리 중인 지 여부확인
            System.out.println("before merge : "+ em.contains(member));
            Object result = em.merge(member);
            tx.commit();
    ...
    void defach(String userId, String password){
        EntityManager em = jpaTemplate.createEntityManager();
        EntityTransaction tx = em.getTransaction();
        ...
        try{
            ...
            System.out.println("after defach : " + em.contains(member));
            tx.commit();
        } catch (Exception e) {
            tx.rollback();
        }finally {
